...
def get_report_summary(report_json):
    ...
    return "Report generated from selected article text."


def get_report_key_points(report_json):
    for section in report_json.get("sections", []):
        if section.get("heading", "").lower() == "key findings":
            return section.get("items", [])

    return []


def get_report_recommendations(report_json):
    for section in report_json.get("sections", []):
...
def create_ai_job(payload, current_user):
    ...
    if action == "analyze" and entity_type == "article":
        ...
        try:
            ...
            result_json = {
                "summary": {
                    "executive": get_report_summary(report_json),
                    "technical": "Structured report generated by Gemini and rendered safely by the dashboard.",
                    "key_points": get_report_key_points(report_json),
                },
                "extracted_cves": [],
                "extracted_iocs": [],
                "extracted_entities": {
                ...
                "attack_mappings": [],
                "recommended_actions": get_report_recommendations(report_json),
                "confidence": 0.7,
                "evidence": ["Report generated from selected article text."],
            }
        except Exception as error:
            result_json = build_mock_ai_result(action, entity_type, entity_id, selected_text)
